--[[
    HuginProcessor.lua - process images via Hugin
...
local function run_hugin_Process()
  ...
  local stagingfolder = df.sanitize_filename( dt.preferences.read( mod, "HuginStagingFolder", "string" ) )
  ...
  if dt.configuration.running_os == 'macos' then
    zerene_commandline = zerene_java_folder .. os_path_seperator .. 'jre' .. os_path_seperator .. 'bin' .. os_path_seperator .. 'java"' -- java runtime packaged with zerene
      ...
      .. ' -Xdock:name="HuginProcessor" -Xdock:icon="' .. zerene_java_folder .. '/../ZereneEurydice.icns"' .. ' -Dapple.laf.useScreenMenuBar=true' -- Settings to integrate in to apple dock etc                            
      .. ' -classpath "' .. zerene_java_folder.. os_path_seperator .. 'HuginProcessor.jar:' -- tell the JRE where to find the Zerene Stacker application and libraries
...
-- run Zerene Stacker
  ...
  if resp ~= 0 then
    dt.print( _'could not start HuginProcessor application' )
  end
  ...
  if lfs_loaded then
    -- use lfs to find all tif images in staging folder - even if default output filenames are changed in HuginProcessor
    for this_file in lfs.dir(stagingfolder) do
      if this_file ~= "." and this_file ~= ".." then
        dt.print_log('Found '.. this_file)
...
-- Preferences - locate executables for Hugin, hugin_executor and pto_gen
...
dt.preferences.register(
  ...
  _('Hugin executable'),	-- label
	_('Select the executable Hugin'),	-- tooltip
  "" -- default,
)
...
dt.preferences.register(
  ...
  "hugin_exec_Bin",	-- name
	"file",	-- type
  _('hugin_executor executable'),	-- label
	_('Select the executable hugin_executor'),	-- tooltip
  "" -- default,
)
...
dt.preferences.register(
  ...
  "pto_gen_Bin",	-- name
	"file",	-- type
  _('pto_gen executable'),	-- label
	_('Select the executable pto_bin'),	-- tooltip
  "" -- default,
)
